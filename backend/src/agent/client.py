...
class AgentClient:
    ...
    def _author_sync(self, dataset_description, defect_type, feedback, guidance, strategy) -> str:
        ...
        prompt = (
            ...
            "Write ONE short inpainting prompt (MAX 15 words) as comma-separated visual "
            "descriptors of the DEFECT ONLY.\n"
            "RULES:\n"
            "- Describe ONLY what the defect looks like (shape, color, texture).\n"
            "- NEVER describe the surrounding surface, material, or background. The inpainting "
            "model preserves the background automatically — describing it causes color shifts.\n"
            "  WRONG: 'scratch on glossy black ceramic surface with matte contrast'\n"
            "  RIGHT: 'thin diagonal scratch, faint silver, slightly raised edges'\n"
            "- Keep it under 15 words. Shorter prompts produce sharper results.\n"
            f"{extra}\n\n"
    ...
    def _derive_profile_sync(self, image_description, dataset_description,
        ...
        prompt = (
            ...
            f"User placement: {placement}\n\n"
            "Do four things:\n\n"
            "A) Group the defects into PLACEMENT BUCKETS.\n"
            "   IMPORTANT — the 'User placement' field above is AUTHORITATIVE. If the user says "
            "defects are 'localised on the via holes' or 'on the teeth', ALL defects go into a "
            "bucket targeting that specific feature. The user's placement overrides any default "
            "reasoning about where defects 'naturally' occur.\n"
            "   If the user gives no specific placement (e.g. just 'anywhere' or empty), group by "
            "where each defect naturally occurs on the object.\n"
            "   Every defect goes in exactly one bucket.\n\n"
            "B) For each bucket pick region_mode + detect_prompts.\n"
            "   DECISION RULE for region_mode:\n"
            "   - 'instance': the bucket's defects target a SPECIFIC, COUNTABLE feature that a "
            "segmentation model can detect individually (holes, pads, vias, pins, rivets, solder "
            "joints). Each detected feature becomes one region. The detect_prompt MUST name that "
            "specific feature shape.\n"
            "   - 'subdivide': the bucket's defects can appear ANYWHERE on a broad surface — they "
            "don't target one specific feature type. This includes scratch, crack, corrosion, rust, "
            "discoloration, staining on any surface. The whole object is gridded into cells.\n"
            "   KEY TEST: if the user's placement names a specific feature (via holes, teeth, pads), "
            "use 'instance' — the user wants defects ON those features. Otherwise ask 'does the "
            "defect go ON a specific small feature, or ANYWHERE on the surface?' If anywhere → "
            "subdivide. If on a specific feature → instance.\n"
            "   Example: user says 'on the via holes' with scratch/crack → instance (user placed "
            "them on specific features, even though scratch can also go anywhere).\n"
            "   Example: user says 'anywhere' with scratch/crack on a gear → subdivide.\n"
            "   Example: imperfect via holes on a PCB → instance (targets specific via holes). "
            "Scratch on a PCB with no placement hint → subdivide (goes anywhere on the board).\n\n"
            "   CRITICAL: detection runs on the CLEAN image BEFORE any defect exists, so "
            "detect_prompts must name the INTACT feature or object the defect will be placed ON — "
            "NEVER the defect itself, NEVER the substrate/board/panel.\n"
            "   detect_prompts MUST match what the user's placement targets:\n"
            "   - If user says 'on the via holes' → detect_prompts target the via holes "
            "('silver circle', 'via hole', 'round pad') — NOT the PCB itself.\n"
            "   - If user says 'on the teeth' → detect_prompts target teeth "
            "('gear tooth', 'metal tooth') — NOT the gear.\n"
            "   - instance: detect_prompts MUST describe the SHAPE of the individual feature the "
            "segmentation model should find. Format: [color/size] + [shape noun]. Shape nouns: "
            "circle, hole, dot, ring, square, tooth, bump, pad. Examples: 'silver circle', "
            "'small round pad', 'copper dot'. WRONG: 'green pcb', 'circuit board', 'copper "
            "surface' — those are substrates, not features. grid = null.\n"
            "   - subdivide: detect_prompts name the whole CLEAN object as a plain noun ('metal "
            "gear', 'steel disc'). Give grid {\"rows\":N,\"cols\":N}, 2-4 each.\n"
            ...
            "   The ONLY question is: does the defect change the object's OUTLINE or SILHOUETTE?\n"
            "   - feasible=true: anything that changes APPEARANCE ONLY within existing geometry — "
            "rust, scratch, crack, discoloration, pitting, burn, staining, imperfect fill, rough "
            "finish, surface contamination, irregular texture. These are paintable.\n"
            "   - feasible=false: ONLY defects that REMOVE MATERIAL or ALTER THE SILHOUETTE — "
            "missing teeth, broken edge, chipped corner, hole where none should be. Inpainting "
            "cannot delete geometry.\n"
            "   When ambiguous (e.g. 'bad manufacture', 'imperfect holes'), default to feasible=true "
            "— surface-quality interpretations are always inpaintable. Give a short reason.\n\n"
